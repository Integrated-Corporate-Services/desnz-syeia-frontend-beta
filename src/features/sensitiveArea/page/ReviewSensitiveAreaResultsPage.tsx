...
const ReviewSensitiveAreaResultsPage: React.FC = () => {
    ...
    const getLayerCategories = () => {
        ...
        return {
            ...
            failedAreas: [...(checks.automated.failed.screeningRequired || []), ...(checks.automated.failed.noScreening || [])],
        };
    };
    ...
    const hasFailedChecks = (): boolean => {
        ...
        const { checks } = checksSummary;
        return (checks.automated.failed.screeningRequired?.length || 0) > 0 || (checks.automated.failed.noScreening?.length || 0) > 0;
    };

    const hasExcessiveFailures = (): boolean => {
        if (!checksSummary) return false;
        const { checks } = checksSummary;
        const totalFailed = (checks.automated.failed.screeningRequired?.length || 0) + (checks.automated.failed.noScreening?.length || 0);
    ...
    const getSuccessfulCheckCount = (): number => {
        ...
        const { checks } = checksSummary;
        return (checks.automated.passed.screeningRequired?.length || 0) + (checks.automated.passed.noScreening?.length || 0) + (checks.automated.cleared.screeningRequired?.length || 0) + (checks.automated.cleared.noScreening?.length || 0);
    };
                ...
                <div className="govuk-grid-column-two-thirds">
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <div className="govuk-inset-text">We could not complete all the sensitive area checks.</div>

                            {/* Next Steps Subheading */}
                            <h2 className="govuk-heading-m">Next steps</h2>

                            {/* Instructions - Wait 24 hours */}
                            <p className="govuk-body">Please wait at least 24 hours and run the sensitive area checks again.</p>

                            {/* Instructions - Contact team */}
                            <p className="govuk-body">
                                Contact our team at{' '}
                                <a href="mailto:37consents@energysecurity.gov.uk" className="govuk-link" aria-label="Email the S37 consents team">
                            ...
                            <p className="govuk-body">
                                <Link to={`${S37_BASE_URL}/${effectiveApplicationId}/task-list`} className="govuk-link">
                                    Return to task list
                                </Link>
                            </p>
                        </>
                        ...
                        <>
                            ...
                            {passedAreasScreening.length > 0 && (
                                <>
                                    <p className="govuk-body">Your route passes through these areas and requires an environmental impact assessment screening. This will add a charge of £60.00 to your application fee:</p>
                            ...
                            {passedAreasNoScreening.length > 0 && (
                                <>
                                    <p className="govuk-body">It also passes through these areas which do not require screening:</p>
                                    <ul className="govuk-list govuk-list--bullet">
