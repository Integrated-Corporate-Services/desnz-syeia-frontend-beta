...
const SupportingInfo: React.FC = () => {
	...
	const [signedWayleaveDate, setSignedWayleaveDate] = useState({ day: "", month: "", year: "" });
	const [signedWayleaveFile, setSignedWayleaveFile] = useState<File | null>(null);
	const [inheritedWayleave, setInheritedWayleave] = useState("");
	const [counterNoticeDate, setCounterNoticeDate] = useState({ day: "", month: "", year: "" });
	const [inheritedWayleaveFile, setInheritedWayleaveFile] = useState<File | null>(null);
	const [anyPayments, setAnyPayments] = useState("");
	const [anyPaymentsFile, setAnyPaymentsFile] = useState<File | null>(null);
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (!signedWayleave) {
			...
			if (!signedWayleaveDate.day || !signedWayleaveDate.month || !signedWayleaveDate.year) {
				newErrors.signedWayleaveDate = "Enter the full 21 day notice date";
			}
			if (!signedWayleaveFile) {
				newErrors.signedWayleaveFile = "Upload a document to support your application";
			}
		}
		// Validate inheritedWayleave
		if (!inheritedWayleave) {
			newErrors.inheritedWayleave = "Select if a counter notice has been given by the landowner";
		} else if (inheritedWayleave === "Yes") {
			if (!counterNoticeDate.day || !counterNoticeDate.month || !counterNoticeDate.year) {
				newErrors.counterNoticeDate = "Enter the full counter notice issue date";
			}
			if (!inheritedWayleaveFile) {
				newErrors.inheritedWayleaveFile = "Upload a document to support your application";
			}
		}
		// Validate anyPayments
		if (!anyPayments) {
			newErrors.anyPayments = "Select if your application includes a title plan";
		} else if (anyPayments === "Yes" && !anyPaymentsFile) {
				...
				<div className="govuk-grid-row">
					<div className="govuk-grid-column-two-thirds">
						...
						<form onSubmit={handleSubmit} noValidate>
							{/* 21 day notice */}
							<div className={`govuk-form-group${errors.signedWayleave ? " govuk-form-group--error" : ""}`}>
								<fieldset className="govuk-fieldset" aria-describedby="signedWayleave-hint">
									...
									<div id="signedWayleave-hint" className="govuk-hint">
										{errors.signedWayleave && (
											<p className="govuk-error-message" id="signedWayleave-error">{errors.signedWayleave}</p>
										)}
									</div>
									<div className="govuk-radios" data-module="govuk-radios">
										<div className="govuk-radios__item">
											<input className="govuk-radios__input" id="signedWayleave" name="signedWayleave" type="radio" value="Yes" checked={signedWayleave === "Yes"} onChange={() => setSignedWayleave("Yes")}/>
											<label className="govuk-label govuk-radios__label" htmlFor="signedWayleave">Yes</label>
										</div>
										<div className="govuk-radios__item">
											<input className="govuk-radios__input" id="signedWayleave-2" name="signedWayleave" type="radio" value="No" checked={signedWayleave === "No"} onChange={() => setSignedWayleave("No")}/>
									...
									{signedWayleave === "Yes" && (
										<div className="govuk-radios__conditional" id="conditional-signedWayleave">
											<div className={`govuk-form-group${errors.signedWayleaveDate ? " govuk-form-group--error" : ""}`}>
												<fieldset className="govuk-fieldset" role="group" aria-describedby="passport-issued-hint">
													...
													{errors.signedWayleaveDate && (
														<p className="govuk-error-message" id="passport-issued-error">{errors.signedWayleaveDate}</p>
													)}
											...
											<div className={`govuk-form-group${errors.signedWayleaveFile ? " govuk-form-group--error" : ""}`}>
												<label className="govuk-label" htmlFor="signedWayleave-upload-1" id="signedWayleave-upload-1-label">Upload a document</label>
												<div id="signedWayleave-upload-1-hint" className="govuk-hint">
													...
													{errors.signedWayleaveFile && (
														<div id="signedWayleave-upload-1-error" className="govuk-error-message">{errors.signedWayleaveFile}</div>
													)}
												</div>
												<input className="govuk-file-upload" id="signedWayleave-upload-1" name="signedWayleaveUpload1" type="file" aria-describedby="signedWayleave-upload-1-hint" onChange={handleFileChange(setSignedWayleaveFile)} />
											</div>
										</div>
									)}
								</fieldset>
							...
							{/* Counter notice */}
							<div className={`govuk-form-group${errors.inheritedWayleave ? " govuk-form-group--error" : ""}`}>
								<fieldset className="govuk-fieldset" aria-describedby="inheritedWayleave-hint">
									...
									<div id="inheritedWayleave-hint" className="govuk-hint">
										{errors.inheritedWayleave && (
											<p className="govuk-error-message" id="inheritedWayleave-error">{errors.inheritedWayleave}</p>
										)}
									</div>
									<div className="govuk-radios" data-module="govuk-radios">
										<div className="govuk-radios__item">
											<input className="govuk-radios__input" id="inheritedWayleave" name="inheritedWayleave" type="radio" value="Yes" checked={inheritedWayleave === "Yes"} onChange={() => setInheritedWayleave("Yes")}/>
											<label className="govuk-label govuk-radios__label" htmlFor="inheritedWayleave">Yes</label>
										</div>
										<div className="govuk-radios__item">
											<input className="govuk-radios__input" id="inheritedWayleave-2" name="inheritedWayleave" type="radio" value="No" checked={inheritedWayleave === "No"} onChange={() => setInheritedWayleave("No")}/>
									...
									{inheritedWayleave === "Yes" && (
										<div className="govuk-radios__conditional" id="conditional-inheritedWayleave">
											<div className={`govuk-form-group${errors.counterNoticeDate ? " govuk-form-group--error" : ""}`}>
												<fieldset className="govuk-fieldset" role="group" aria-describedby="counter-issued-hint">
													<legend className="govuk-fieldset__legend govuk-fieldset__legend--l">
														<h1 className="govuk-label govuk-label--s">Counter notice issue date</h1>
													</legend>
													<div id="counter-issued-hint" className="govuk-hint">For example, 27 3 2007</div>
													{errors.counterNoticeDate && (
														<p className="govuk-error-message" id="counter-notice-error">{errors.counterNoticeDate}</p>
													)}
											...
											<div className={`govuk-form-group${errors.inheritedWayleaveFile ? " govuk-form-group--error" : ""}`}>
												<label className="govuk-label" htmlFor="inheritedWayleave-upload-1" id="inheritedWayleave-upload-1-label">Upload a document</label>
												<div id="inheritedWayleave-upload-1-hint" className="govuk-hint">
													<p className="govuk-hint">You can upload .pdf, .jpg, .jpeg, .png, .msg,  .doc, .docx, .xls, and .xlsx files of up to 25MB each. Files cannot be password-protected.</p>
													{errors.inheritedWayleaveFile && (
														<div id="inheritedWayleave-upload-1-error" className="govuk-error-message">{errors.inheritedWayleaveFile}</div>
													)}
												</div>
												<input className="govuk-file-upload" id="inheritedWayleave-upload-1" name="inheritedWayleaveUpload1" type="file" aria-describedby="inheritedWayleave-upload-1-hint" onChange={handleFileChange(setInheritedWayleaveFile)} />
											</div>
										</div>
									)}
								</fieldset>
							</div>
							{/* Any payments (title plan) */}
							<div className={`govuk-form-group${errors.anyPayments ? " govuk-form-group--error" : ""}`}>
								<fieldset className="govuk-fieldset" aria-describedby="anyPayments-hint">
									...
									<div id="anyPayments-hint" className="govuk-hint">
										{errors.anyPayments && (
											<p className="govuk-error-message" id="anyPayments-error">{errors.anyPayments}</p>
										)}
									</div>
									<div className="govuk-radios" data-module="govuk-radios">
										<div className="govuk-radios__item">
											<input className="govuk-radios__input" id="anyPayments" name="anyPayments" type="radio" value="Yes" checked={anyPayments === "Yes"} onChange={() => setAnyPayments("Yes")}/>
											<label className="govuk-label govuk-radios__label" htmlFor="anyPayments">Yes</label>
										</div>
										<div className="govuk-radios__item">
											<input className="govuk-radios__input" id="anyPayments-2" name="anyPayments" type="radio" value="No" checked={anyPayments === "No"} onChange={() => setAnyPayments("No")}/>
									...
									{anyPayments === "Yes" && (
										<div className="govuk-radios__conditional" id="conditional-anyPayments">
											<div className={`govuk-form-group${errors.anyPaymentsFile ? " govuk-form-group--error" : ""}`}>
												<label className="govuk-label" htmlFor="anyPayments-upload-1" id="anyPayments-upload-1-label">Upload the title plan document</label>
												<div id="anyPayments-upload-1-hint" className="govuk-hint">
													<p className="govuk-hint">You can upload .pdf, .jpg, .jpeg, .png, .msg,  .doc, .docx, .xls, and .xlsx files of up to 25MB each. Files cannot be password-protected.</p>
													{errors.anyPaymentsFile && (
														<div id="anyPayments-upload-1-error" className="govuk-error-message">{errors.anyPaymentsFile}</div>
													)}
												</div>
												<input className="govuk-file-upload" id="anyPayments-upload-1" name="anyPaymentsUpload1" type="file" aria-describedby="anyPayments-upload-1-hint" onChange={handleFileChange(setAnyPaymentsFile)} />
											</div>
										</div>
									)}
								</fieldset>
