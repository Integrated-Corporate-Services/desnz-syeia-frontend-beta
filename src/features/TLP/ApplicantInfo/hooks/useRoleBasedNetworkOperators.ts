...
const logger = createLogger("useRoleBasedNetworkOperators-TLP");

/**
 * Custom hook to fetch role-based network operators for TLP dropdown
  ...
  useEffect(() => {
    const fetchOperators = async () => {
      try {
        setLoading(true);
        setError("");
        logger.debug("Fetching role-based network operators for TLP");
        ...
        logger.debug("Network operators fetched successfully for TLP", {
        ...
        logger.error("Error fetching network operators for TLP", { error: errorMsg });
        setError(errorMsg);
      } finally {
        setLoading(false);
      }
