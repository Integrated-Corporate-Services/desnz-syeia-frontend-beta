...
const logger = createLogger("useRoleBasedNetworkOperators-NWL");

/**
 * Custom hook to fetch role-based network operators for NWL dropdown
  ...
  useEffect(() => {
    const fetchOperators = async () => {
      try {
        setLoading(true);
        setError("");
        logger.debug("Fetching role-based network operators for NWL");
        ...
        logger.debug("Network operators fetched successfully for NWL", {
        ...
        logger.error("Error fetching network operators for NWL", { error: errorMsg });
        setError(errorMsg);
      } finally {
        setLoading(false);
      }
